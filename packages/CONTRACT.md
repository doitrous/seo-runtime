...
| `GET /api/seo/health` | **Bearer secret** | `{version, siteSlug, lastSyncAt, snapshotVersion, counts, redirectHits: [{source, hits}]}`. It names the site and enumerates every redirect source path, and it is the only route whose response resets state, so it is never anonymous. |
